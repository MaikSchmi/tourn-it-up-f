...
function AuthContextWrapper(props) {
  ...
  const loginUser = async (email, password, {justSignedUp}) => {
    try {
      const token = await axios.post(`${import.meta.env.BASE_URL}/auth/login`, {
        email: email ,
        password: password,
      })
  ...
  const renewToken = async () => {
    try {
      const token = await axios.post(`${import.meta.env.BASE_URL}/auth/update-token`, {
        email: user.email
      })
  ...
  const authenticateUser = async () => {
    ...
    if (storedToken) {
      try {
        const response = await axios.get(`${import.meta.env.BASE_URL}/auth/verify`, {
          headers : {
