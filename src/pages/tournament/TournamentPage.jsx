...
function TournamentPage() {
  ...
  const addParticipant = async () => {
    try {
      const addedTournament = await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/updateparticipants/${id}`, {user: user, signup: true})
  ...
  const removeParticipant = async () => {
    try {
      const resignedTournament = await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/updateparticipants/${id}`, {user: user, resign: true})
  ...
  const deleteConfirmed = async () => {
    try {
      await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/delete/${id}`, {user: user});
      navigate("/home");
    } catch(error) {
      console.log(error);
    }
  ...
  const addComment = async () => {
    try {
      await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/comments/add`, {comment: comment, username: user.username, tournamentId: tournament._id});
      setComment("");
      getTournamentInfo();
    } catch (error) {
  ...
  const deleteComment = async (id) => {
    try {
      await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/comments/delete/${id}`, {tournamentId: tournament._id});
      getTournamentInfo();
    } catch (error) {
  ...
  const handleBgImageUpload = async (e) => {
    ...
    try {
      const uploadedFile = await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/upload/${tournament._id}`, formData, {withCredentials: true});
  ...
  const handleSaveCustomizeChanges = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.BASE_URL_API}/tournaments/update-values/${tournament._id}`, {textColor: textColor, backgroundColor: backgroundColor + alphaValue});
